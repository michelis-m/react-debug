...
function Main(props) {
  ...
  const fetchRandomStatistics = useCallback(() => {
    ...
    const oneYearSeconds = 60 * 60 * 24 * 30;
    let curProfit = Math.round(2000 + Math.random() * 1000);
    ...
    for (let i = 0; i < iterations; i += 1) {
      ...
      curProfit += Math.round((Math.random() * 2 - 1) * 100);
      curViews += Math.round((Math.random() * 2 - 1) * 100);
